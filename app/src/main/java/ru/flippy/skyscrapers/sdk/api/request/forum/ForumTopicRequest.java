        ...
        RetrofitClient.getApi().forumTopic(topicId, pageNumber)
                ...
                .success(new SourceCallback() {
                    ...
                    public void onResponse(Source doc) {
                        ...
                        topic.setBan(!doc.select("a[class=bl amount cntr][href*=bans]:contains(Вы не можете комментировать этот топик)").isEmpty());
                        topic.setClosed(!doc.select("div[class=cntr amount m5]:contains(топик закрыт)").isEmpty()
                                && !doc.select("span[class=bl amount cntr]:contains(Вы не можете комментировать топик, потому что тема закрыта)").isEmpty());
                        List<Comment> comments = new ArrayList<>();
                        for (Element commentElement : doc.select("ul.msgs>li")) {
                        ...
                        listener.onResponse(topic, doc.pagination());
                    }
                });
    }
}
