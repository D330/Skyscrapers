        ...
        RetrofitClient.getApi().profile(userId)
                ...
                .success(new SourceCallback() {
                    ...
                    public void onResponse(Source doc) {
                        ...
                        Element headerElement = doc.select("div.snow:contains(«):contains(»):contains(Уровень:):has(span.user)").first().parent();
                        ...
                        profile.setCupCount(doc.select("div.cntr>a[href*=/cup]>span").size());
                        List<Award> awards = new ArrayList<>();
                        for (Element awardElement : doc.select("div>div.nfl.cntr>div.inbl.txtal")) {
                        ...
                        if (cityElement != null) {
                            ...
                            Element budgetElement = doc.select("span.minor:contains(Внесено в бюджет:)").first();
                            ...
                            cityInfo.setGuildInvite(!self && !doc.select("a[class*=bl link tdn][href*=guildInviteLink]").isEmpty());
                            cityInfo.setRole(roleElement.text());
                            if (statusElement != null) {
                                cityInfo.setStatus(statusElement.text());
                            }
                            cityInfo.setExperience(doc.select("span.minor:contains(Опыт города:)").first().parent().select("span>last-child").text());
                            cityInfo.setDayCount(Integer.parseInt(doc.select("span.minor:contains(Дней в городе:)").first().parent().select("span:last-child").text()));
                            ...
                            cityInfo.setBirthday(!doc.select("div[class*=nfl balls]>strong:contains(Отмечает день города!").isEmpty());
                            profile.setCityInfo(cityInfo);
                        }
                        if (self) {
                            profile.setFriend(false);
                            profile.setSex(doc.select("div.cntr>span:has(img[src*=/user/]):has(span.user):contains(" + SkyscrapersSDK.getUserNick() + ")").last().select("img").first().attr("alt").equals("м") ? User.sex.MAN : User.sex.WOMAN);
                            profile.setOnline(User.online.ONLINE);
                        } else {
                            profile.setFriend(doc.select("a[class*=bl link tdn][href*=addFriendLink]").isEmpty());
                            Element onlineElement = doc.select("span.minor:contains(Статус:)").first().nextElementSibling();
                            ...
                            if (onlineElement.text().equals("онлайн")) {
                                ...
                                String[] lastEntranceArray = doc.select("span.minor:contains(Посл. вход:)").first().nextElementSibling().text().split(" ");
                        ...
                        //Parse personal data
                        Elements personalElements = doc.select("div.m5:has(b[class*=bl cntr]):contains(Персонал)>div:has(span.amount)>span[class*=small flr amount]");
                        ...
                        Elements businessElements = doc.select("div.m5:has(b[class*=bl cntr]:contains(Бизнес))>a[href*=/business/]");
                        ...
                        //Parse cars
                        Element carsElement = doc.select("div.cntr:has(div.m5:contains(Техника:)):has(a.m5[href*=/auto/])").first();
                        if (carsElement != null) {
                        ...
                        Element giftsElement = doc.select("a[class=m5 bl link tdn][href*=/gift/]:contains(Подарки:)>span[class=small flr buff]>span").first();
                        if (giftsElement != null) {
                            profile.setGiftCount(Integer.parseInt(giftsElement.text()));
                        }
                        Bonus bonus = new Bonus();
                        Element bonusElement = doc.select("div[class=nfl cntr small ny]:has(span[class=amount nwr]):has(span.amount):contains(Текущая выручка):contains(Рекорд)").first();
                        ...
                        Element experienceElement = doc.select("span.minor:contains(Опыт:)").first();
                        ...
                        profile.setGameDayCount(Integer.parseInt(doc.select("span.minor:contains(Дней в игре:)").first().nextElementSibling().text()));
                        Element aboutElement = doc.select("span.minor:contains(О себе:)").first();
                        if (aboutElement != null) {
                            profile.setAbout(aboutElement.nextElementSibling().text());
                        }
                        Element marriageElement = doc.select("span[class=white tdn]:contains(В браке с:)").first();
                        if (marriageElement != null) {
                            ...
                            marriage.setNewlyweds(!doc.select("div[class=nfl ny]>div.hearts>strong>span:contains(Молодожены!)").isEmpty());
                            profile.setMarriage(marriage);
                        }
                        profile.setBirthday(!doc.select("div[class=nfl ny balls]>div.m5>strong>span:contains(День рождения!)").isEmpty());
