...
public class ProfileRequest extends BaseRequest {
    ...
    public void execute(final OnRequestListener<Profile> listener) {
        DocumentLoader.connect("http://nebo.mobi/tower/id/" + userId)
                .execute(new OnDocumentListener() {
                    ...
                    public void onResponse(Document document, long wicket) {
                        ...
                        if (parser.checkPageError()) {
                            ...
                            boolean self = SkyscrapersAPI.getUserId() == userId;
                            ...
                            if (self) {
                                ...
                                profile.setSex(document.select("div.cntr>span:has(img[src*=/user/]):has(span.user):contains(" + SkyscrapersAPI.getUserNick() + ")").last().select("img").first().attr("alt").equals("м") ? User.sex.MAN : User.sex.WOMAN);
