    ...
    public void execute(final RequestListener<Profile> listener) {
        RetrofitClient.getApi().profile(userId)
                ...
                .success(profileDoc -> {
                    ...
                    Element headerElement = profileDoc.select("div.snow:contains(«):contains(»):contains(Уровень:):has(span.user)").first().parent();
                    ...
                    profile.setCupCount(profileDoc.select("div.cntr>a[href*=/cup]>span").size());
                    List<Award> awards = new ArrayList<>();
                    for (Element awardElement : profileDoc.select("div>div.nfl.cntr>div.inbl.txtal")) {
                    ...
                    if (cityElement != null) {
                        ...
                        Element budgetElement = profileDoc.select("span.minor:contains(Внесено в бюджет:)").first();
                        ...
                        cityInfo.setGuildInvite(!self && !profileDoc.select("a[class*=bl link tdn][href*=guildInviteLink]").isEmpty());
                        cityInfo.setRole(roleElement.text());
                        if (statusElement != null) {
                            cityInfo.setStatus(statusElement.text());
                        }
                        cityInfo.setExperience(profileDoc.select("span.minor:contains(Опыт города:)").first().parent().select("span>last-child").text());
                        cityInfo.setDayCount(Integer.parseInt(profileDoc.select("span.minor:contains(Дней в городе:)").first().parent().select("span:last-child").text()));
                        ...
                        cityInfo.setBirthday(!profileDoc.select("div[class*=nfl balls]>strong:contains(Отмечает день города!").isEmpty());
                        profile.setCityInfo(cityInfo);
                    }
                    if (self) {
                        profile.setFriend(false);
                        profile.setSex(profileDoc.select("div.cntr>span:has(img[src*=/user/]):has(span.user):contains(" + SkyscrapersSDK.getUserNick() + ")").last().select("img").first().attr("alt").equals("м") ? User.sex.MAN : User.sex.WOMAN);
                        profile.setOnline(User.online.ONLINE);
                    } else {
                        profile.setFriend(profileDoc.select("a[class*=bl link tdn][href*=addFriendLink]").isEmpty());
                        Element onlineElement = profileDoc.select("span.minor:contains(Статус:)").first().nextElementSibling();
                        ...
                        if (onlineElement.text().equals("онлайн")) {
                            ...
                            String[] lastEntranceArray = profileDoc.select("span.minor:contains(Посл. вход:)").first().nextElementSibling().text().split(" ");
                    ...
                    //Parse personal data
                    Elements personalElements = profileDoc.select("div.m5:has(b[class*=bl cntr]):contains(Персонал)>div:has(span.amount)>span[class*=small flr amount]");
                    ...
                    Elements businessElements = profileDoc.select("div.m5:has(b[class*=bl cntr]:contains(Бизнес))>a[href*=/business/]");
                    ...
                    //Parse cars
                    Element carsElement = profileDoc.select("div.cntr:has(div.m5:contains(Техника:)):has(a.m5[href*=/auto/])").first();
                    if (carsElement != null) {
                    ...
                    Element giftsElement = profileDoc.select("a[class=m5 bl link tdn][href*=/gift/]:contains(Подарки:)>span[class=small flr buff]>span").first();
                    if (giftsElement != null) {
                        profile.setGiftCount(Integer.parseInt(giftsElement.text()));
                    }
                    Bonus bonus = new Bonus();
                    Element bonusElement = profileDoc.select("div[class=nfl cntr small ny]:has(span[class=amount nwr]):has(span.amount):contains(Текущая выручка):contains(Рекорд)").first();
                    ...
                    Element experienceElement = profileDoc.select("span.minor:contains(Опыт:)").first();
                    ...
                    profile.setGameDayCount(Integer.parseInt(profileDoc.select("span.minor:contains(Дней в игре:)").first().nextElementSibling().text()));
                    Element aboutElement = profileDoc.select("span.minor:contains(О себе:)").first();
                    if (aboutElement != null) {
                        profile.setAbout(aboutElement.nextElementSibling().text());
                    }
                    Element marriageElement = profileDoc.select("span[class=white tdn]:contains(В браке с:)").first();
                    if (marriageElement != null) {
                        ...
                        marriage.setNewlyweds(!profileDoc.select("div[class=nfl ny]>div.hearts>strong>span:contains(Молодожены!)").isEmpty());
                        profile.setMarriage(marriage);
                    }
                    profile.setBirthday(!profileDoc.select("div[class=nfl ny balls]>div.m5>strong>span:contains(День рождения!)").isEmpty());
